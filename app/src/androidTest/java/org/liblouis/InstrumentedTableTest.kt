...
class InstrumentedTableTest {

    private lateinit var louis: Louis
    private lateinit var translationTableList: TranslationTableList

    private val uebTablePath = "/data/user/0/org.liblouis.test/app_louis/liblouis/tables/en-ueb-g2.ctb"
    private val uebTable = TranslationTable(uebTablePath)
    ...
    fun listTables() {
        val expectation = """
            ...
            """.trimIndent()
        val tableNames = translationTableList.tables.map { it.fileName }.toString()
        assertEquals(tableNames, expectation)
    }

    @Test
    fun checkTranslationTablesExist() {
        translationTableList.tables.forEach {
            val file = it.file
            assert(file.exists()) { "Table file not found: ${file.absolutePath}" }
        }
    }

    @Test
    fun checkTranslationTablesUnique() {
        val paths = translationTableList.tables.map { it.filePath }
        val duplicates = paths.groupingBy { it }.eachCount().filter { it.value > 1 }.keys
        assert(duplicates.isEmpty()) { "Duplicate table files: $duplicates" }
    }

    // TODO: Make tests to test these with example values

    @Test
    fun testUebTranslations() {
        translateBraille(uebTable, "hello")
        translateBraille(uebTable, "you")
    }
